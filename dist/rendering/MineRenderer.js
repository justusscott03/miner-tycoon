...
export class MineRenderer {
    ...
    drawForeground() {
        const s = this.state;
        this.shaftRenderers.forEach(r => r.display());
        ...
        this.carrierRenderers.forEach(r => r.display());
        fill(0);
        textAlign("CENTER", "CENTER");
        text(MoneyState.Instance.total, canvas.width / 2, 50);
        image(ImageManager.Instance.get("topBottomGradient"), 0, 0, canvas.width, 50);
        image(ImageManager.Instance.get("topBottomGradient"), 0, canvas.height - 50, canvas.width, 50);
    }
    display(delta) {
