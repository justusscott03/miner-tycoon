...
export class MineState {
    ...
    constructor() {
        ...
        this.elevator = new ElevatorState(
            95,
            this.storehouse.y + this.storehouse.h + 31,
            110,
            170,
            this.shafts.map(s => s.crate),
            this.storehouse
        );
    ...
    buildShaft() {
        if (this.numShafts < 30) {
            const shaft = new ShaftState(
                215,
                740 + this.shaftOffset,
                500,
                100,
                this.numShafts + 1
            );
    ...
    recruitCarrier() {
        if (this.numCarriers < 5) {
            const carrier = new CarrierState(
                450,
                375,
                52.15,
                75,
    ...
    static fromJSON(data: MineSaveData): MineState {
        ...
        mine.elevator = ElevatorState.fromJSON(
            data.elevator,
            mine.shafts.map(s => s.crate),
            mine.storehouse
        );

        mine.numCarriers = data.numCarriers;
        mine.carriers = data.carriers.map(c =>
            CarrierState.fromJSON(c, mine.storehouse, mine.warehouse)
        );
