...
import { BaseLayer } from "../tools/Shape Editor/Layers/BaseLayer";
import { Layer } from "../tools/Shape Editor/Layers/Layer";
import { GroupLayer } from "../tools/Shape Editor/Layers/GroupLayer";

import { ToolManager } from "../tools/Shape Editor/ToolManager";
import { EditorCanvasManager } from "../tools/Shape Editor/EditorCanvasManager";
import { SelectionManager } from "../tools/Shape Editor/SelectionManager";
import { ShapeFactory } from "../tools/Shape Editor/ShapeFactory";
import { HierarchyPanel } from "../tools/Shape Editor/HierarchyPanel";
import { InspectorPanel } from "../tools/Shape Editor/InspectorPanel";
import { ContextMenu } from "../tools/Shape Editor/ContextMenu";
import { RenderLoop } from "../tools/Shape Editor/RenderLoop";
import { Exporter } from "../tools/Shape Editor/Exporter";
import { TransformGizmo } from "../tools/Shape Editor/TransformGizmo";
...
export class ShapeEditorWindow {
    layers: BaseLayer[] = [];
    ...
    refresh() {
        this.hierarchy.render(
            ...
            (x, y, layer) => this.contextMenu.show(
                x,
                y,
                layer.id,
                this.selection.selectedLayers.length,
    ...
    findLayerAtPoint(mouse: { x: number; y: number }): Layer | null {
    ...
    flattenLayers(nodes: BaseLayer[]): Layer[] {
        const result: Layer[] = [];
        for (const node of nodes) {
            if (node instanceof Layer) result.push(node);
            else if (node.isGroup()) {
                result.push(...this.flattenLayers((node as GroupLayer).children));
            }
        }
        return result;
    }

    findLayerById(id: string): BaseLayer | null {
        return this.findLayerByIdRecursive(id, this.layers);
    }

    findLayerByIdRecursive(id: string, nodes: BaseLayer[]): BaseLayer | null {
        for (const node of nodes) {
            if (node.id === id) return node;
            if (node.isGroup()) {
                const found = this.findLayerByIdRecursive(id, (node as GroupLayer).children);
    ...
    ungroupSelected() {
        if (!this.selection.isSingleGroupSelected) return;

        const group = this.selection.selectedLayers[0] as GroupLayer;
